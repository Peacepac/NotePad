...
public class DetailActivity extends AppCompatActivity {
    ...
    private void saveNote() {
        ...
        if (title.length() != 0) {
            ...
            contentValues.put(NoteDBContract.NoteEntry.COLUMN_CONTENT, content);
            contentValues.put(NoteDBContract.NoteEntry.COLUMN_TITLE, title);

            long insert = database.insert(NoteDBContract.NoteEntry.TABLE_NAME, null, contentValues);
            if (insert > 0) {
                Toast.makeText(getApplicationContext(), "Record saved sucessfully :" + insert, Toast.LENGTH_SHORT).show();
            }
            finish();
        } else {
